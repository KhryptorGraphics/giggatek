...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function updateSegmentFilters() {
        // Clear existing options (except "No segment")
        ...
        state.segments.forEach(segment => {
            ...
            option.textContent = `${segment.name} (${segment.member_count} members)`;
            campaignSegment.appendChild(option);
        });
    }
    ...
    function loadCampaigns() {
        ...
        fetch(`/api/ab-testing/campaigns?${params.toString()}`, {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${getAuthToken()}`
    ...
    function renderCampaigns() {
        ...
        campaignsTableBody.innerHTML = state.campaigns.map(campaign => `
            <tr>
                <td>${campaign.id}</td>
                <td>${campaign.name}</td>
                <td>${campaign.segment_name || 'All Users'}</td>
                <td>${campaign.variant_count}</td>
                <td>
                ...
                <td>
                    <span class="badge badge-${getStatusBadgeClass(campaign.status)}">${campaign.status}</span>
                </td>
                <td>
                    <button type="button" class="btn btn-sm btn-primary edit-campaign-btn" data-id="${campaign.id}">Edit</button>
                    <button type="button" class="btn btn-sm btn-secondary variants-btn" data-id="${campaign.id}">Variants</button>
                    <button type="button" class="btn btn-sm btn-info results-btn" data-id="${campaign.id}">Results</button>
        ...
        document.querySelectorAll('.variants-btn').forEach(btn => {
            btn.addEventListener('click', () => {
                const campaignId = btn.getAttribute('data-id');
                alert('Variants management will be implemented in the next step');
            });
        });
        ...
        document.querySelectorAll('.results-btn').forEach(btn => {
            btn.addEventListener('click', () => {
                const campaignId = btn.getAttribute('data-id');
                alert('Results view will be implemented in the next step');
            });
        });
    ...
    function openCreateCampaignModal() {
        ...
        campaignForm.reset();
        campaignId.value = '';
        campaignModalTitle.textContent = 'Create A/B Test Campaign';
    ...
    function openEditCampaignModal(id) {
        ...
        // Set modal title
        campaignModalTitle.textContent = 'Edit A/B Test Campaign';
    ...
    function saveCampaign() {
        ...
        // Add segment ID if selected
        if (campaignSegment.value) {
            campaignData.segment_id = parseInt(campaignSegment.value);
        }
        ...
        const endpoint = isCreating ? '/api/ab-testing/campaigns' : `/api/ab-testing/campaigns/${campaignId.value}`;
    ...
    function deleteCampaign(id) {
        ...
        fetch(`/api/ab-testing/campaigns/${id}`, {
            method: 'DELETE',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${getAuthToken()}`
